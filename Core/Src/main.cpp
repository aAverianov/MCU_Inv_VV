...
enum status {
	DEVICE_ON,		// (bit 0) сигнал "Сеть"
	WORK,			// (bit 1) сигнал "Работа"
	ALARM,			// (bit 2) сигнал "Авария"
	REMOTE,			// (bit 3) режим "Местн./Дистанц."
	COMMUNICATION,	// (bit 4) сигнал "Проверка связи"
	STATUS_ALARM_1,	// (bit 5) код аварийной ситуации
	STATUS_ALARM_2,	// (bit 6) 
...
const uint16_t timing3 = 50;	// 50 ms для
...
const uint16_t timer2s = 2000;			// 3000 ms
const uint16_t timer3s = 3000;			// 500 ms
...
int main(void)
{
  ...
  // подготовка IP-адреса
	...
	led.SetTiming(timeToContact, timeCommunication, timing3, timing4, timing5);	// установки таймингов 0,1,2,3,4 (значения см.выше)
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOC, SCS_Pin|RST_Pin, GPIO_PIN_RESET);
